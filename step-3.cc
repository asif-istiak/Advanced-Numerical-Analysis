...
private:
  void make_grid(); // creates triangulation corresponds to unit square
  void setup_system(); // distributes the DOF and matrixes
  void assemble_system();
  void solve();
  void output_results() const; // provides output file
  ...
  Triangulation<Asif> triangulation;
  FE_Q<Asif>          fe;
  DoFHandler<Asif>    dof_handler; //keeps track of DOF
...
void Step3<Asif>::assemble_system()
{
  ...
  // present problem:
  QGauss<Asif> quadrature_formula(fe.degree + 1);
...
void Step3<Asif>::output_results() const
{
  ...
  // formats):
  std::ofstream output( Asif==2 ? "solution-2d.vtk": "solution-3d.vtk" );
...
void Step3<Asif>::run()
{
  std::cout << "Solving problem in" << Asif << "space dimension."
  <<std::endl;
...
{
  Step3<2> laplace_problem_2d;
  laplace_problem_2d.run();
}
{
  Step3<3> laplace_problem_3d;
